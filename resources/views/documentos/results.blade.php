  ...
  <div class="section">
    <div class="col m11 s12">
      <div class="row">
        ...
        @can('upload')
          <a href="/documents/create" class="btn waves-effect waves-light right tooltipped" data-position="left" data-delay="50" data-tooltip="Upload New Document"><i class="material-icons">arquivo_upload</i></a>
        @endcan
        </h3>
      ...
      <div class="card z-depth-2">
        <div class="card-content">
          <div class="row">
            <div class="left">
              <h6 class="flow-text orange-text">{{ count($results) }} Resultados</h6>
            </div>
            <div class="search input-field">
              <form action="/search" method="post" id="search-form">
                {{ csrf_field() }}
                  <i class="material-icons prefix">search</i>
                  <input type="text" name="search" id="search" placeholder="procurar aqui ...">
          ...
          @if(count($results) > 0)
            @foreach($results as $res)
            	@foreach($res as $r)
		              ...
		              <a href="documents/{{ $r->id }}">
		                <div class="card hoverable indigo lighten-5task" data-id="{{ $r->id }}">
            ...
            @endforeach
          @else
            <h5 class="teal-text">Nenhuma equivalência encontrada :(</h5>
          @endif
          </div>
        </div>
      </div>
